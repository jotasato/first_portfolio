class User < ApplicationRecord
  ...
  validates :height, :presence => true, :on => :record
  validates :weight, :presence => true, :on => :record
  validates :age, :presence => true, :on => :record
  ...
  def maintain_calories(params, momentum) 
    ...
    if params[:gender] == "male" 
        self.result = (66.0 + (13.7 * params[:weight].to_i) + (5.0 * params[:height].to_i) - (6.8 *params[:age].to_i)) * momentum.to_f
      else
        # 女の子だった場合はこちらで計算
